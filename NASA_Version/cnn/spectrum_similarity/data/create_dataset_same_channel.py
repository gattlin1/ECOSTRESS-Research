        ...
        for i in range(len(folder_files)):
            ...
            for j in range(i, len(folder_files)):
                if types_count[spectrum_type] < count_per_type: # to ensure there isn't a same file ab pair
                    ...
                    a = cv2.split(img_array_1)

                    a1 = np.where(img_array_1 != [255, 255, 255])
                    a2 = np.where(img_array_2 != [255, 255, 255])
                    a3 = np.zeros(img_array_1.shape, dtype='uint8')

                    #combined = cv2.add(img_array_1, img_array_2)
                    combined = cv2.merge((a1, a2, a3))
                    cv2.imshow('1', img_array_1)
                    cv2.imshow('2', img_array_2)

                    cv2.imshow('comb', combined)
                    cv2.waitKey(0)


                    # b,g,r = cv2.split (img_array_1)
...
if __name__=='__main__':
    ...
    matching_data = get_matching_entries(files, categories.index('match'), 100)
    print(f'Matching entries {len(matching_data)}')

    nonmatching_data = get_nonmatching_entries(files, categories.index('non-match'), 1)
